"""
Worker configuration — settings from environment variables.

On import, values from a ``.env`` file beside this module are loaded into
``os.environ`` (via python-dotenv). If a variable is already set in the
process environment (e.g. Railway), that value wins and is not overwritten
by the file.
...
# ``worker/.env`` next to this file — works regardless of shell cwd when using
# ``python -m worker`` from ``src/jobs/worker``.
