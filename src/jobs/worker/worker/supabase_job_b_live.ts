 ...
 *
 * Three-tier Redis writes (split by match liveness):
 *   BASE (every tick / 30s, ALL fixtures — NS and live):
 *     gojo:live:fixture:{id}:base         → pre-match + mostly-static details
 *                                           (referee, venue, date, league,
 *                                            teams, lineups, coaches, status)
 *
 *   LIGHT (every tick / 30s, LIVE fixtures only):
 ...
 *
 *   HEAVY (every 5 elapsed minutes, LIVE fixtures only):
 ...
 *
 * Liveness classification (must match worker/db.py LIVE_STATUSES):
 *   live     = 1H, HT, 2H, ET, BT, P, LIVE
 *   anything else (NS, TBD, FT, AET, PEN, CANC, PST, ...) = not live.
 * For non-live fixtures only the base key is written; once status flips to
 * live, light + heavy kick in automatically.
 *
 * Request body:
 ...
 *     "ok": true,
 *     "fixture_statuses": [...],
 *     "redis_writes": 12,         // total across base + light + heavy
 *     "base_writes": 4,
 *     "light_writes": 6,
 *     "heavy_writes": 2,
 *     "heavy_fetches": 2
...
// Must mirror worker/db.py LIVE_STATUSES exactly.
const LIVE_STATUSES = new Set<string>(["1H", "HT", "2H", "ET", "BT", "P", "LIVE"]);

function isLive(statusShort: string | null | undefined): boolean {
  return !!statusShort && LIVE_STATUSES.has(statusShort);
}
...
// ---------------------------------------------------------------------------
// Build base snapshot from API fixture (base write — pre-match + static data)
//
// Contains fields that are either fixed for the match (venue, date, referee,
// league, teams) or change only pre-kickoff (lineups, coaches, status).  The
// `status` block is included so CANC/PST/SUSP/INT transitions are visible to
// consumers reading only the base key.
// ---------------------------------------------------------------------------

// deno-lint-ignore no-explicit-any
function buildBaseSnapshot(f: ApiFixture): Record<string, any> {
  return {
    fixture_id: f.fixture.id,
    ...
    periods: f.fixture.periods ?? null,
...
// ---------------------------------------------------------------------------
// Write base snapshots to Redis (every tick, ALL fixtures)
//
// Overwrites the base key each tick so late pre-match mutations (late lineup
// edits, referee updates, status transitions to CANC/PST/SUSP before kickoff)
// are always reflected.  Idempotent once the match goes live because these
// fields stop mutating from then on.
// ---------------------------------------------------------------------------

async function writeBaseBuckets(fixtures: ApiFixture[]): Promise<number> {
  if (fixtures.length === 0) return 0;

  const commands: (string | number)[][] = [];
  for (const f of fixtures) {
    const baseKey = `${KEY_PREFIX}:${f.fixture.id}:base`;
    const base = buildBaseSnapshot(f);
    commands.push(["SET", baseKey, JSON.stringify(base), "EX", REDIS_TTL_SEC]);
  }
...
// ---------------------------------------------------------------------------
// Write light snapshots to Redis (every tick, LIVE fixtures only)
// ---------------------------------------------------------------------------

async function writeLightBuckets(fixtures: ApiFixture[]): Promise<number> {
  const liveFixtures = fixtures.filter((f) => isLive(f.fixture.status?.short));
...
// ---------------------------------------------------------------------------
// Write heavy snapshots to Redis (every 5 elapsed minutes, LIVE fixtures only)
...
async function writeHeavyBuckets(fixtures: ApiFixture[]): Promise<{ commands: number; fetches: number }> {
  const heavyFixtures = fixtures.filter(
    (f) => isLive(f.fixture.status?.short) && isHeavyTick(f.fixture.status.elapsed),
  );
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. Base Redis writes (every tick, ALL fixtures — NS + live)
    const baseCount = await writeBaseBuckets(apiFixtures);
    console.log(`Base writes: ${baseCount} commands`);

    // 3. Light Redis writes (every tick, LIVE fixtures only)
    const lightCount = await writeLightBuckets(apiFixtures);
    console.log(`Light writes: ${lightCount} commands`);

    // 4. Heavy Redis writes (every 5 elapsed minutes, LIVE fixtures only)
    ...
    // 5. Extract and return per-fixture statuses
    ...
    return new Response(
      JSON.stringify({
        ...
        redis_writes: baseCount + lightCount + heavy.commands,
        base_writes: baseCount,
        light_writes: lightCount,
        heavy_writes: heavy.commands,
